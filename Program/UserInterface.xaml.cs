        ...
        private void ReadMatrixFromGrid()
        {
            ...
            for (int i = 0; i < n; i++)
            {
                for (int j = 0; j < n; j++)
                {
                    ...
                    {
                        throw new MatrixException($"Комірка [{i + 1}, {j + 1}] містить неприпустиме значення (NaN або нескінченність).");
                    }
            ...
            if (e.EditAction == DataGridEditAction.Commit)
            {
                ...
                if (textBox != null)
                {
                    ...
                    if (!string.IsNullOrEmpty(text))
                    {
                        ...
                        {
                            ShowError("Значення не може бути NaN або нескінченністю.");
                            e.Cancel = true;
                            return;
                        }

                        if (val != 0 && (Math.Abs(val) > 1000000 || Math.Abs(val) < 1e-5))
                        {
